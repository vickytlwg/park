...
chargeApp.controller("feeDetailCtrl", ['$scope', '$http',  'textModal', '$modal', '$timeout', function($scope, $http, textModal,$uibModal, $timeout){
	...
	$scope.detail.getPage = function(){		
		...
		$http.post('page', {low:($scope.detail.page.index - 1) * $scope.detail.page.size, count:$scope.detail.page.size})
		.success(function(response){
...
feeDetail.service('textModal',  ['$uibModal', function($uibModal){
