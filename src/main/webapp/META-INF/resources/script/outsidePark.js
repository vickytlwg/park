(function($) {
	...
	var bindRefreshClick = function() {
		...
		refreshBtn.on('click', $(this), function() {
			renderPagination();
			renderPark($.fn.page.pageSize * ($.fn.page.currentPage - 1),
					$.fn.page.pageSize);
			$(this).blur();
		});
	...
	var authoritySetBtnClickHandle = function() {
		...
		var checkedTr = $('#parkBody').find('input[type="checkbox"]:checked')
				.parents('tr');
	...
	var searchParkByLocation = function() {
		$('#submitSearchParkBtn').on('click', $(this), function() {
			var locationKey = {
				'keywords' : $('input#locationKeyWords').val()
			};
		...
		$.ajax({
			...
			success : function(data) {
				if (data.status == 1001) {
					...
					$('select#feeCriterion').append(
							'<option value=20>未绑定</option>');
					for (var i = 0; i < criterions.length; i++) {
						$('select#feeCriterion').append(
								'<option value=' + criterions[i].id + '>'
										+ criterions[i].name + '</option>');
					}
	...
	var fillAuthorityBody = function(data) {
		$.each(data, function(index, authorityTmp) {
			if (authorityTmp['channel'] == 1) {
				...
				$('#month1expire')
						.prop("checked", authorityTmp['monthexpired']);
	...
	var assignAddParkForm = function(checkedTr) {
		...
		$('input#isFree')[0].checked = parseInt($(tds[9]).attr('data')) == 1 ? true
				: false;
											...
											if (data['status'] = 1001) {
												...
												addParkResultDiv
														.append($.fn.tip
																.success('提交操作完成'));
												setTimeout(
														'$("#addParkResult").html(""); $("#modalCloseBtn").click();$("#refresh").click()',
														500);
											}
										},
										error : function(data) {
											loader.remove();
											addParkResultDiv.append($.fn.tip
													.error('提交操作未完成'));
											setTimeout(
													'$("#addParkResult").html("");',
													3000);
	...
	var getAddParkFormValue = function() {
		...
		if ($('#isPositionChange').get(0).checked) {
			parkFields['position'] = $('#zoneCenterId').find("option:selected")
					.text()
					+ '-'
					+ $("#areaId").find("option:selected").text()
					+ '-'
					+ $("#streetid").find("option:selected").text();
		}
	...
	var fillParkTbody = function(data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			if (data[i]['streetName'] != undefined) {
				tr.append('<td streetId=' + data[i]['streetId'] + '>'
						+ data[i]['streetName'] + '</td>');
			} else {
				tr.append('<td >' + '' + '</td>');
			}
			/*
			...
			tr.append('<td data=' + feeCriterionId + ' >' + feeCriterionName
					+ '</td>');
			tr.attr('feeCriterionId', feeCriterionId);

			var status = data[i]['status'] == 0 ? '可用' : '不可用';
			tr
					.append('<td data=' + data[i]['status'] + ' >' + status
							+ '</td>');
			var free = parseInt(data[i]['isFree']) == 1 ? '是' : '否';
			// tr.append('<td data=' + data[i]['isFree'] + ' >' + free +
			// '</td>');
			// tr.append('<td> style=“display:none” ' + data[i]['floor']+
			// '</td>');
			...
			// tr.append('<td hidden longitude='+ data[i]['longitude'] +'
			// latitude=' + data[i]['latitude'] + ' >' + data[i]['position']+
			// '</td>');
			...
			if (picUri != undefined && picUri != null && picUri != "")
				tr
						.append('<td><a href="#" ref="' + picUri
								+ '"> 显示图片</a></td>');
			else
				tr.append('<td>暂无图片</td>');
			tr.append("<td style='display:none'>" + data[i]['streetId']
					+ "</td>");
			tr.find('a').on(
					'click',
					$(this),
					function() {
						var modal = new $.Modal('showPic', "停车场图片",
								'<center><img width=300 height=300 src="'
										+ $(this).attr("ref")
										+ '"></img></center>');
			...
			tr
					.append('<tr><td><a href="#">修改</a></td><td>|</td><td><a href="#">删除</a></td></tr>');
			parkBody.append(tr);
		}
	};
