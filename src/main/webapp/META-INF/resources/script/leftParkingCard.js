(function($){
	...
	var renderChannelCarport = function(low, count){
		...
		var parkId = $('select#searchPark').val();
			if (parkId<1) {
				url=$.fn.config.webroot + "/getchannelDetailByMacType?low=" + low + "&count=" + count  +"&macType=2&_t=" + (new Date()).getTime();
			} else {
...
//		alert(data.length);
	...
	var fillMacInfo = function(additionalOption){
		...
		$.ajax({
			url:$.fn.config.webroot + '/getUnBoundHardwares/2?_t=' + (new Date()).getTime(),
			type: 'get',
			contentType: 'application/json;charset=utf-8',
			success: function(data){successFunc(data);},
			error: function(data){errorFunc(data);}
	...
	var renderPagination = function(){
		...
		$.ajax({
			url:$.fn.config.webroot + "/getchannelCountByMacType/2",
			type: 'get',
			success: function(data){
				data = $.parseJSON(data["body"]);
				var paginationUl = $.fn.page.initial(data['count'], pageClickFunc);
