...
(function ($) {
    $.jBox = function (content, options) {
        ...
        boxHtml.push('<a class="jbox-close" title="' + $.jBox.languageDefaults.close + '" onmouseover="$(this).addClass(\'jbox-close-hover\');" onmouseout="$(this).removeClass(\'jbox-close-hover\');" style="position:absolute; display:block; cursor:pointer; top:' + (7 + options.border) + 'px; right:' + (12 + options.border) + 'px; width:19px; height:20px;' + (options.showClose ? '' : 'display:none;') + '"></a>');

        if (withTitle) {
            boxHtml.push('<div class="jbox-title-panel" style="height:35px;">');
            boxHtml.push('<div class="jbox-title' + (options.showIcon == true ? ' jbox-title-icon' : (options.showIcon == false ? '' : ' ' + options.showIcon)) + '" style="float:left; width:' + titleWidth + '; line-height:' + ($.browser.msie ? 34 : 34) + 'px; padding-left:' + (options.showIcon ? 35 : 5) + 'px;overflow:hidden;text-overflow:ellipsis;word-break:break-all;">' + (options.title == '' ? '&nbsp;' : options.title) + '</div>');
            boxHtml.push('</div>');
        }
        ...
        var ieButtonFix = $.browser.msie ? 'line-height:19px;padding:0px 6px 0px 6px;' : 'padding:0px 6px 0px 10px;';
        $.each(content, function (stateName, stateOptions) {
            ...
            states.push('<div class="jbox-button-panel" style="height:45px;padding:3px 0 8px 0;text-align: right;' + (withButtons ? '' : 'display:none;') + '">');
            if (!options.isTip) {
            ...
            $.each(stateOptions.buttons, function (k, v) {
                ...
                if (number == 1) {
                    ...
                    states.push('<button class="jbox-buttonCannel" value="' + v + '" style="' + ieButtonFix + '">' + k + '</button>');
                }
                number++;
            });
            //states.push('<button class="jbox-button" value="true" style="' + ieButtonFix + '">退出</button>');
            //states.push('<button class="jbox-buttonCannel" value="false" style="' + ieButtonFix + '">取消</button>');
            states.push('</div></div>');
        });
    ...
    $.jBox.prompt = function (content, title, icon, options, width, height) {
        var defaults = {
            top: "25%",
            content: content,
            title: title,
        ...
        var top = options.title == undefined ? 69 : 69;
        ...
        var html = [];
        html.push('html:');
        html.push('<div style="font-size:16px;font-family:\'Microsoft YaHei\';margin-top:37px;' + minHeight + 'padding-left:' + (options.icon == 'none' ? 0 : 70) + 'px;text-align:left;">');
        html.push('<span class="jbox-icon jbox-icon-' + options.icon + '" style="position:absolute; top:' + (top + options.border) + 'px;left:' + (25 + options.border) + 'px; width:32px; height:32px;"></span>');
    ...
    $.jBox.confirm = function (content, title, submit, options,width,height) {
        var defaults = {
        ...
        options = $.extend({}, defaults, options);
        $.jBox.prompt(content, title, 'question', options, width, height);
    };
    ...
    $.jBox.tip = function (content, icon, options,width,height, timeout, opacity) {
        var defaults = {
            width: width,
            height: height,
            content: content,
        ...
        var html = [];
        html.push('html:');
        html.push('<div style="min-height:18px;height:auto;margin:10px;padding-left:55px;padding-top:15px;text-align:left;">');
        html.push('<span class="jbox-icon jbox-icon-' + options.icon + '" style="position:absolute;top:' + (18 + options.border) + 'px;left:' + (20 + options.border) + 'px; width:32px; height:32px;"></span>');
        html.push(options.content);
        html.push('</div>');
        options.content = html.join('');
...
_jBoxConfig.defaults = {
    ...
    opacity: 0.4, /* 窗口隔离层的透明度,如果设置为0,则不显示隔离层 */
...
function openhtml(iframe, title, width, heigh) {
    jBox.open("iframe:" + iframe, title, width, heigh);
}
function alertsuccess(title, width, height, timeout) {
    jBox.tip(title, 'success', '', width, height, timeout,0.4);
}
function alerttips(title, width, height, timeout, opacity) {
    jBox.tip(title, 'success', '', width, height, timeout, opacity);
}
function alertLoading(title, width, height) {
    jBox.tip(title, 'loading','', width, height,0, 0.2);
}
function closeLoading() {
    jBox.closeTip();
}
function alertconfirm(count, title) {
    ...
    jBox.confirm(count, title, submit, { showScrolling: false, buttons: { '确认': true, '取消': false } });
}
function alertwarning(count, title) {
    var submit = function (v, h, f) {
        if (v == 'yes') {
            jBox.tip('已保存。');
        }
    ...
    jBox.warning(count, title, submit);
}